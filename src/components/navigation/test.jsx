			...
			<div className='w-full relative py-3 '>
				<AnimatePresence>
					...
					{openMenu && (
						...
						>
							<button onClick={handleCloseMenu} className='fixed top-[30px] right-[30px]'>
							...
							{navlinks.map((item, index) => {
								const openValue = item.name === 'Products' ? products : services
								if (item.type === 'dropdown') {
										...
										<div key={index} className=''>
											...
											<div
												className={`  transition-all grid  ease-in-out duration-500   ${
													openValue ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'
												} `}
											>
									...
									return (
										<NavLink onClick={handleCloseMenu} key={index} to={item.link}>
											{item.name}
										</NavLink>
									)
								}
							...
							<button onClick={handleCloseMenu} className='bg-transparent outline-none border-none'>
								Apply as a talent
							</button>
							<a
								onClick={handleCloseMenu}
				...
				<div className=' w-[90vw] bg-white border-[0.5px] border-[#F1F5FA] rounded-[15px] tablet:rounded-[25px] max-w-[1280px] mx-auto flex justify-between items-center py-[11px] px-4 tablet:py-4 tablet:px-[20px] laptop:px-[30px]'>
								...
								if (item.type === 'dropdown') {
									...
									return (
										<NavLink
											...
											to={item.link}
											className={({ isActive }) =>
												`transition-all ease-linear duration-200 ${isActive ? ' text-[#0967D2]  ' : ' text-[#67737E]'} `
											}
										>
											{item.name}
										</NavLink>
					...
					<div className=' hidden laptop:flex justify-center items-center gap-5'>
						...
						>
							<p className='text-[#0967D2]  text-[15px] leading-5'>Talk to us</p>
